...
interface MMCFormProps {
  onCalculate: (params: MMCParams) => void;
}

const MMCForm: React.FC<MMCFormProps> = ({ onCalculate }) => {
  ...
  function isValid() {
    if (!lambda || !mu || !servers) return false;
    const lambdaNum = parseFloat(lambda);
    const muNum = parseFloat(mu);
    const serversNum = parseInt(servers);
    if (isNaN(lambdaNum) || isNaN(muNum) || isNaN(serversNum)) return false;
    if (lambdaNum <= 0 || muNum <= 0 || serversNum <= 0) return false;
  ...
  function handleChangeLambda(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    validate(e.target.value, mu, servers);
  }

  function handleChangeMu(e: React.ChangeEvent<HTMLInputElement>) {
    setMu(e.target.value);
    validate(lambda, e.target.value, servers);
  }

  function handleChangeServers(e: React.ChangeEvent<HTMLInputElement>) {
    setServers(e.target.value);
    validate(lambda, mu, e.target.value);
  }

  function validate(lambdaVal: string, muVal: string, serversVal: string) {
    if (!lambdaVal || !muVal || !serversVal) {
    ...
    if (isNaN(lambdaNum) || isNaN(muNum) || isNaN(serversNum)) {
      setError("Ingrese valores numéricos válidos.");
      return;
    }
    if (lambdaNum <= 0 || muNum <= 0 || serversNum <= 0) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col items-center w-full">
      ...
      <Button
        type="submit"
        className="bg-blue-600 hover:bg-blue-800 mt-6 w-[40rem]"
        disabled={!isValid()}
        variant="default"
      >
        Calcular
